...
// Space Game
//TODO: power ups, boss fight, alter ship controls, explode location?,
...
void draw() {   
  ...
  if (s1.alive == true) {
    ...
    for (int i = asteroids.size()-1; i>=0; i--) {
      ...
      for (int j = lasers.size()-1; j >= 0; j--) {
        ...
        if (dist(l.x, l.y, a.x, a.y) < a.r) {
          ...
          if (asteroids.get(i).health < 1 ) {
            ...
            ////randomly add a powerup
            //if (random(101)<15) {
            //  for (int p = pu.size()-1; p>=0; p--) {
            //    pu.add(new PowerUp( asteroids.get(i).x, asteroids.get(i).y-20, int(random(4))));
            //  }
            //}
      ...
      // laser and power up collision detection
      //for (int p = pu.size()-1; p>=0; p--) {
      //  if (dist(l.x, l.y, pu.get(p).x, pu.get(p).y) < 10) {
      //    switch(pu.get(p).type) {
      //    case 1:
      //      s1.health += 20;
      //      break;
      //    case 2:
    ...
    // j instead of i here vvv???

    // Display and Dispose Lasers
    for (int j = lasers.size()-1; j>=0; j--) {
      //for (int j = 0; j<lasers.size(); j++) {
      lasers.get(j).shoot();
      lasers.get(j).display();
      if (lasers.get(j).reachedTop()) {
        lasers.remove(j);
      }
    }

    //// if you enable this, be sure to disable the other one
    ...
    ////display and dispose of Power Ups
    //for (int p = pu.size()-1; p>=0; p--) {
    //  pu.get(p).move();
    //  switch(pu.get(p).type) {
    //  case 1:
...
void gameStart() {
  ...
  text("High Score: " + tr1.getString("SCORE") + " - " + tr1.getString("NAME"), width/2, 0+height/5 );
